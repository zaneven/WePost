...
const card = JSON.parse(readFileSync('/tmp/wepost-card-dsv4.json', 'utf8'));
...
if (hasTitle) {
  ...
  if (el) {
    await el.screenshot({ path: '/tmp/wepost-card-dsv4.png', type: 'png' });
    console.log("Card screenshot saved!");
  }
} else {
  await page.screenshot({ path: '/tmp/wepost-card-dsv4.png', type: 'png', fullPage: false });
