...
| `content` | string | 正文，支持轻量 Markdown（见下）。段间空一行 |
...
普通段落内部的单个换行会渲染为软换行（不断段）。**关键**：列表项之间不要留空行，否则会被拆成多个独立块导致间距异常。
...
→ 识别为「金句/思考」，选 `zen-quote` / `3:4`。生成 CardData（title 取核心句的精炼、正文保留原文并用 `>` 包裹金句、author 默认 `WePost 研习社`、date 取当天、watermarkText 取前 4 字印章），写 JSON、跑脚本、开浏览器，回复展示 JSON 与 URL。
